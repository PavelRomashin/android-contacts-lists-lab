...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String[] Jlo = ContactType.getContactTypes(); // создайте массив строк. Для инициализации используйте метод ContactType.getContactTypes(),
        // который должен возвращать список всех доступных источников контактов

        ArrayList<FilterContactTypeUi> NewOneList = new ArrayList<>();// создайте список типа FilterContactTypeUi и заполните его с помощью цикла forEach
        NewOneList.add(createAllSelectedItem(Jlo));// forEach должен бежать по строковому массиву, который вы создали ранее
        for (String type : Jlo) {
            NewOneList.add(createFilterContactType(type));
        }
        filterContactTypesLiveDate.setValue(NewOneList);
    }
